...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Convert LVIS detection json to Ultralytics YOLO txt labels.")
    parser.add_argument("--ann-json", required=True)
    parser.add_argument("--image-root", required=True, help="COCO root that contains train2017/ and val2017/.")
    parser.add_argument("--output-root", required=True)
    parser.add_argument("--split", required=True, help="lvis_train or lvis_val, only used for folder naming.")
    return parser.parse_args()


def resolve_rel_path(im: dict) -> str:
    ...
    if coco_url:
        ...
        if len(parts) >= 2:
            return "/".join(parts[-2:])   # train2017/xxx.jpg or val2017/xxx.jpg

    raise KeyError(f"Neither 'file_name' nor usable 'coco_url' found for image id={im.get('id')}")
...
def main() -> None:
    ...
    with open(image_list_path, "w", encoding="utf-8") as list_f:
        for im in images:
            ...
            rel_path = resolve_rel_path(im)
            image_path = Path(args.image_root) / rel_path
            if not image_path.exists():
                missing += 1

            list_f.write(str(image_path.resolve()) + "\n")

            label_path = labels_dir / (Path(rel_path).stem + ".txt")
    ...
    if missing:
        print(f"Warning: {missing} image paths listed do not currently exist on disk.")
